...
const SheetTrigger = ({ asChild, children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { asChild?: boolean }) => { const state = React.useContext(SheetContext); const onClick = () => state?.setOpen(true); const child = children as React.ReactElement<Record<string, unknown>>; return asChild ? React.cloneElement(child, { ...props, onClick, className: cn(props.className, child.props.className as string | undefined) }) : <button type="button" {...props} onClick={onClick}>{children}</button> }
const SheetClose = ({ asChild, children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { asChild?: boolean }) => { const state = React.useContext(SheetContext); const onClick = () => state?.setOpen(false); const child = children as React.ReactElement<Record<string, unknown>>; return asChild ? React.cloneElement(child, { ...props, onClick }) : <button type="button" {...props} onClick={onClick}>{children}</button> }
...
const SheetContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { side?: "top" | "bottom" | "left" | "right"; sheetTitle?: string; sheetDescription?: string; onCloseAutoFocus?: (event: React.FocusEvent<HTMLElement>) => void }>(({ side = "right", className, children, sheetTitle, sheetDescription, onCloseAutoFocus: _onCloseAutoFocus, ...props }, ref) => { const state = React.useContext(SheetContext); if (!state?.open) return null; const placement = { top: "inset-x-0 top-0 border-b", bottom: "inset-x-0 bottom-0 border-t", left: "inset-y-0 left-0 h-full w-3/4 border-r", right: "inset-y-0 right-0 h-full w-3/4 border-l" }[side]; return <SheetPortal><SheetOverlay onClick={() => state.setOpen(false)} /><div ref={ref} role="dialog" aria-label={sheetTitle ?? sheetDescription} onClick={(event) => event.stopPropagation()} className={cn("fixed z-[70] gap-4 bg-background p-6 shadow-lg sm:max-w-sm", placement, className)} {...props}>{children}</div></SheetPortal> })
