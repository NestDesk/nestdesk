...
const DropdownMenuLabel = ({ className, inset, ...props }: React.HTMLAttributes<HTMLDivElement> & { inset?: boolean }) => <div className={cn("px-2 py-1.5 text-sm font-semibold", inset && "pl-8", className)} {...props} />
...
const DropdownMenuGroup = passthrough; const DropdownMenuPortal = passthrough; const DropdownMenuSub = passthrough; const DropdownMenuSubContent = DropdownMenuContent; const DropdownMenuSubTrigger = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => <div {...props}>{children}<ChevronRight className="ml-auto h-4 w-4" /></div>; const DropdownMenuRadioGroup = passthrough
