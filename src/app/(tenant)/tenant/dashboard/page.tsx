...
export default async function TenantDashboardPage() {
  ...
  return (
    <div className="space-y-2">
      ...
      <Card className="rounded-3xl border border-primary/15 bg-gradient-to-br from-background via-primary/8 to-blue-500/10 shadow-sm">
        <CardHeader className="pb-3">
          <p className="text-[11px] uppercase tracking-[0.22em] text-primary">Profile completion</p>
          <div className="flex items-start justify-between gap-3">
            <CardTitle className="mt-1 text-lg font-semibold text-foreground">Account Status & Profile Completion</CardTitle>
          ...
          <div className="rounded-3xl border border-emerald-500/20 bg-gradient-to-r from-emerald-500/8 via-background to-background p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-start gap-3">
                <div className="rounded-2xl bg-emerald-500/10 p-2 text-emerald-600 dark:text-emerald-400"><Sparkles className="h-4 w-4" /></div>
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Account status</p>
                  <p className="mt-1 text-sm font-semibold text-foreground">{statusCfg.summary}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{statusCfg.description}</p>
                </div>
              </div>
              <span className={`inline-flex items-center gap-1 whitespace-nowrap rounded-full border px-2.5 py-1 text-[11px] font-semibold ${statusCfg.badgeClassName}`}><StatusIcon className={`h-3.5 w-3.5 ${statusCfg.color}`} />{statusCfg.label}</span>
            </div>
          </div>
          <div className="rounded-3xl border border-border/70 bg-background/90 p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-start gap-3">
                <div className="rounded-2xl bg-primary/10 p-2 text-primary"><ShieldCheck className="h-4 w-4" /></div>
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Profile completion</p>
                  <p className="mt-1 text-sm font-semibold text-foreground">{completion.completeCount}/{completion.totalCount} steps completed</p>
                </div>
              </div>
              <span className="rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-semibold text-primary">Completed</span>
            </div>
            <div className="mt-4 h-2.5 w-full overflow-hidden rounded-full bg-muted/80"><div className="h-full rounded-full bg-gradient-to-r from-blue-600 via-cyan-500 to-emerald-500 transition-all" style={{ width: `${completion.percentage}%` }} /></div>
            <p className="mt-3 text-xs text-red-500">{completion.completeCount === completion.totalCount ? null : `${completion.totalCount - completion.completeCount} step(s) still need attention.`}</p>
            {completion.completeCount !== completion.totalCount && <Link href="/tenant/profile" className="mt-3 inline-block"><button className="rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-primary/90">Complete Profile</button></Link>}
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="rounded-3xl border border-primary/15 bg-gradient-to-br from-background via-primary/8 to-blue-500/10 shadow-sm sm:col-span-2 lg:col-span-2">
          <CardHeader className="pb-2">
            <p className="text-[11px] uppercase tracking-[0.22em] text-primary">Property details</p>
            <CardTitle className="mt-1 text-lg font-semibold text-foreground">{hostel?.name ?? "Property details"}</CardTitle>
            {hostel?.property_type ? <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{hostel.property_type}</p> : null}
            {hostel?.address ? <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{[hostel.address, hostel.city, hostel.state, hostel.pincode].filter(Boolean).join(", ")}</p> : null}
            <div className="mt-4 flex flex-wrap items-center gap-2.5 text-sm">
              <span className="inline-flex items-center rounded-full border border-blue-500/40 bg-blue-500/15 px-3 py-1.5 font-semibold text-blue-700 shadow-sm dark:border-blue-400/40 dark:bg-blue-400/15 dark:text-blue-300">
                <CalendarClock className="mr-1.5 h-4 w-4" />
                Member since: {memberSince}
              </span>
              <span className="inline-flex items-center rounded-full border border-amber-500/45 bg-amber-500/15 px-3 py-1.5 font-semibold text-amber-800 shadow-sm dark:border-amber-400/40 dark:bg-amber-400/15 dark:text-amber-300">
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            <div className="rounded-3xl border border-border/70 bg-background/90 p-4 shadow-sm"><div className="flex items-center gap-3"><div className="rounded-2xl bg-primary/10 p-2 text-primary"><Home className="h-4 w-4" /></div><div><p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Assigned room</p><p className="mt-1 text-base font-semibold text-foreground">{room ? `Room ${room.room_number}` : "Not assigned yet"}</p></div></div><p className="mt-3 text-xs text-muted-foreground">{room ? `Capacity: ${room.capacity} occupants` : "Room details will appear here once assigned."}</p></div>
            <div className="rounded-3xl border border-border/70 bg-background/90 p-4 shadow-sm"><div className="flex items-center gap-3"><div className="rounded-2xl bg-amber-500/10 p-2 text-amber-600 dark:text-amber-300"><ShieldCheck className="h-4 w-4" /></div><div><p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Security deposit</p><p className="mt-1 text-base font-semibold text-foreground">{securityDeposit != null ? `₹${securityDeposit}` : "-"}</p></div></div><p className="mt-3 text-xs text-muted-foreground">{securityDeposit != null ? "Security deposit amount on record" : "Security deposit not set yet"}</p></div>
          </CardContent>
          <CardContent className="pt-0"><div className="rounded-3xl border border-border/70 bg-background/90 p-4 shadow-sm"><div className="flex items-center gap-2"><div className="rounded-2xl bg-primary/10 p-2 text-primary"><Megaphone className="h-4 w-4" /></div><div><p className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Latest notice</p><p className="text-sm font-semibold text-foreground">{latestNotice?.title ?? "No published notice yet"}</p></div></div><p className="mt-3 whitespace-pre-wrap text-xs leading-5 text-muted-foreground">{latestNotice?.body ?? "Your property owner’s latest notice will appear here once it is published."}</p></div></CardContent>
        </Card>
